...
async function cleanSeedData() {
  ...
  // Delete agent financial records before deleting agents (FK constraints)
  const seedAgentIds = (await prisma.agent.findMany({
    where: { phone1: { startsWith: "+2340000" } },
    select: { id: true },
  })).map((a) => a.id);
  if (seedAgentIds.length > 0) {
    await prisma.agentLedgerEntry.deleteMany({ where: { agentId: { in: seedAgentIds } } });
    await prisma.settlementAdjustment.deleteMany({ where: { agentId: { in: seedAgentIds } } });
    await prisma.agentSettlement.deleteMany({ where: { agentId: { in: seedAgentIds } } });
  }
  ...
  if (extraAgentIds.length > 0) {
    await prisma.agentLedgerEntry.deleteMany({ where: { agentId: { in: extraAgentIds } } });
    await prisma.settlementAdjustment.deleteMany({ where: { agentId: { in: extraAgentIds } } });
    await prisma.agentSettlement.deleteMany({ where: { agentId: { in: extraAgentIds } } });
  ...
  if (seedProductIds.length > 0) {
    ...
    await prisma.stockAdjustmentItem.deleteMany({ where: { productId: { in: seedProductIds } } });
    await prisma.productOffer.deleteMany({ where: { productId: { in: seedProductIds } } });
    await prisma.productCombo.deleteMany({ where: { productId: { in: seedProductIds } } });
    await prisma.productGift.deleteMany({ where: { productId: { in: seedProductIds } } });
  ...
  // Delete records created by seed users (createdById FK) before deleting users
  if (seedUserIds.length > 0) {
    await prisma.expense.deleteMany({ where: { createdById: { in: seedUserIds } } });
    await prisma.stockAdjustment.deleteMany({ where: { createdById: { in: seedUserIds } } });
    await prisma.journalEntry.deleteMany({ where: { createdById: { in: seedUserIds } } });
    // Stock movements created by seed users (items cascade via schema)
    ...
    await prisma.stockTransfer.deleteMany({ where: { createdById: { in: seedUserIds } } });
    await prisma.purchaseOrder.deleteMany({ where: { createdById: { in: seedUserIds } } });
    // Delete remaining orders (with dependencies) whose salesRepId is a seed user
    const remainingOrderIds = (await prisma.order.findMany({
      where: { salesRepId: { in: seedUserIds } },
      select: { id: true },
    })).map((o) => o.id);
    if (remainingOrderIds.length > 0) {
      const remainingInvoices = (await prisma.invoice.findMany({
        where: { orderId: { in: remainingOrderIds } },
        select: { id: true },
      })).map((i) => i.id);
      if (remainingInvoices.length > 0) {
        await prisma.invoiceItem.deleteMany({ where: { invoiceId: { in: remainingInvoices } } });
        await prisma.invoice.deleteMany({ where: { id: { in: remainingInvoices } } });
      }
      await prisma.delivery.deleteMany({ where: { orderId: { in: remainingOrderIds } } });
      await prisma.pickPack.deleteMany({ where: { orderId: { in: remainingOrderIds } } });
...
async function main() {
  ...
  const inSI = await Promise.all([
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SI-001", type: "INCOMING", status: "RECEIVED",
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SI-002", type: "INCOMING", status: "RECEIVED",
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SI-003", type: "INCOMING", status: "RECEIVED",
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SI-004", type: "INCOMING", status: "RECEIVED",
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SI-005", type: "INCOMING", status: "RECEIVED",
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SI-006", type: "INCOMING", status: "RECEIVED",
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SI-007", type: "INCOMING", status: "RECEIVED",
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SI-008", type: "INCOMING", status: "RECEIVED",
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SI-009", type: "INCOMING", status: "RECEIVED",
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SI-010", type: "INCOMING", status: "RECEIVED",
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SI-012", type: "INCOMING", status: "RECEIVED",
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SI-013", type: "INCOMING", status: "RECEIVED",
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SI-014", type: "INCOMING", status: "RECEIVED",
  ...
  await Promise.all([
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SO-001", type: "OUTGOING", status: "SHELVED",
        warehouseId: whLagos.id, toAgentId: agentOla.id,
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SO-002", type: "OUTGOING", status: "SHELVED",
        warehouseId: whAbuja.id, toAgentId: agentQudus.id,
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SO-003", type: "OUTGOING", status: "SHELVED",
        warehouseId: whPH.id, toAgentId: agentSunmi.id,
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SO-004", type: "OUTGOING", status: "SHELVED",
        warehouseId: whLagos.id, toAgentId: agentFlymack.id,
        state: "Lagos State", country: "Nigeria",
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SO-005", type: "OUTGOING", status: "SHELVED",
        warehouseId: whLagos.id, toAgentId: agentOla.id,
        state: "Lagos State", country: "Nigeria",
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SO-006", type: "OUTGOING", status: "SHELVED",
        warehouseId: whLagos.id, toAgentId: agentQudus.id,
        state: "Lagos State", country: "Nigeria",
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SO-007", type: "OUTGOING", status: "SHELVED",
        warehouseId: whAbuja.id, toAgentId: agentSunmi.id,
        state: "Oyo State", country: "Nigeria",
    ...
    prisma.stockMovement.create({
      data: {
        referenceNumber: "SEED-SO-012", type: "OUTGOING", status: "SHELVED",
        warehouseId: whLagos.id, toAgentId: agentFlymack.id,
        state: "Kaduna State", country: "Nigeria",
  ...
  // ── Materialize StockLevel balances from the seeded movements ──────────────
  const { rebuildStockLevels } = await import("../modules/inventory/services/stock-level.service");
  const rebuilt = await rebuildStockLevels();
  console.log(`  ✓ Stock level rows populated (${rebuilt.rows})`);
