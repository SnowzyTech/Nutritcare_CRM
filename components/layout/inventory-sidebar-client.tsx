  ...
  return (
    <aside className="w-[280px] h-screen bg-white border-r border-gray-100 flex flex-col shrink-0 py-8 px-4 overflow-y-auto no-scrollbar">
      ...
      <div className="flex items-center gap-4 px-4 mb-10">
        <div className="w-12 h-12 rounded-full overflow-hidden bg-purple-100 flex items-center justify-center shrink-0">
          {user.avatarUrl ? (
            ...
            <span className="text-[#9D00FF] font-bold text-sm">{user.initials}</span>
          )}
        </div>
        <div>
          <h3 className="font-bold text-gray-900 text-base">{user.name}</h3>
          <p className="text-gray-400 text-xs font-medium">{formatRole(user.role)}</p>
        </div>
      </div>
      ...
      <div className="mt-8 pt-6 border-t border-gray-50 space-y-2">
        ...
        <button
          onClick={() => onLogout()}
          className="w-full flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-200 text-gray-400 hover:bg-red-50 hover:text-red-600 group"
        >
