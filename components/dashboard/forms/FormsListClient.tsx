                ...
                <div className="flex flex-col gap-3 pt-1">
                  {hasUpsell ? (
                    (data.upsellItems as Array<any>).map((item: any, idx: number) => {
                      const upsellIframe = `<iframe data-nc-id="${form.id}-upsell-${idx}" src="${origin}/order-form/${form.id}?tab=upsell&index=${idx}" width="100%" style="border:none; overflow:hidden; min-height:500px; display:block;" frameborder="0" scrolling="no"></iframe>\n<script>window.addEventListener('message',function(e){if(e.origin!=='${origin}')return;if(e.data&&e.data.type==='nc-resize'){var f=document.querySelector('iframe[data-nc-id="${form.id}-upsell-${idx}"]');if(f)f.style.height=e.data.height+'px';}if(e.data&&e.data.type==='nc-redirect'&&e.data.url){window.location.href=e.data.url;}});<\/script>`;
                      return (
