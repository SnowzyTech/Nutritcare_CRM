    ...
    <>
      <div className="bg-[#FAFAFA] min-h-screen">
        ...
        <div className="px-8 py-8">
          ...
          {good.rapsItems.length > 0 && (
            <div className="mt-8">
              <div className="flex items-center gap-3 mb-3">
                <h3 className="text-[13px] font-semibold text-gray-700">Returned at Point of Supply</h3>
                <span
                  className={`px-2.5 py-0.5 rounded text-[10px] font-semibold uppercase tracking-wide ${
                    good.rapsApprovalStatus === "APPROVED"
                      ? "bg-emerald-100 text-emerald-700"
                      : good.rapsApprovalStatus === "REJECTED"
                      ? "bg-red-100 text-red-600"
                      : "bg-amber-100 text-amber-700"
                  }`}
                >
                  {good.rapsApprovalStatusLabel}
                </span>
              </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200">
                      <th className="px-4 py-2 text-[11px] font-medium text-left text-gray-500">Product</th>
                      <th className="px-4 py-2 text-[11px] font-medium text-right text-gray-500 w-28">Qty Returned</th>
                    </tr>
                  </thead>
                  <tbody>
                    {good.rapsItems.map((item, idx) => (
                      <tr key={idx} className="border-b border-gray-50 last:border-0 bg-white">
                        <td className="px-4 py-2.5 text-[12px] text-gray-600">{item.product}</td>
                        <td className="px-4 py-2.5 text-[12px] text-gray-600 text-right">{item.quantity}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
              {good.rapsApprovalStatus === "PENDING_APPROVAL" && (
                <p className="text-[11px] text-amber-600 mt-2">Awaiting Inventory Manager approval.</p>
              )}
              {good.rapsApprovalStatus === "REJECTED" && good.rapsRejectionReason && (
                <p className="text-[11px] text-red-500 mt-2">Rejected: {good.rapsRejectionReason}</p>
              )}
            </div>
          )}
