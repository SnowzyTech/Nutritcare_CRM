...
const mockOutgoingStock = outgoingStockData;
  ...
  return (
    <div className="flex-1 flex items-center justify-center" style={{ minHeight: "60vh" }}>
      <div className="bg-white rounded-2xl border border-gray-100 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] flex flex-col items-center gap-3 px-20 py-12 min-w-[360px]">
        ...
        <p className="text-xs text-gray-400">Create Outgoing Stock</p>
...
export default function OutgoingStockPage() {
  ...
  const filtered = mockOutgoingStock.filter(
    (r) =>
  ...
  return (
    <div className="max-w-[1400px] mx-auto flex flex-col relative">
      ...
      {/* Toolbar always visible */}
      <Toolbar
        onAdd={() => router.push("/inventory/outgoing/create")}
        search={search}
        onSearch={setSearch}
      />
      ...
      {hasData ? (
        <>
          <DataTable rows={filtered} onRowClick={(id) => router.push(`/inventory/outgoing/${id}`)} />
