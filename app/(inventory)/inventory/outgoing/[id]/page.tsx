...
import { outgoingStockData } from "@/lib/mock-data/outgoing-stock";
...
export default function OutgoingDetailPage({
  ...
  const record = outgoingStockData.find((r) => r.id === Number(id));
    ...
    <div className="max-w-[1400px] mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        ...
        <div className="bg-gray-50 rounded-xl border border-gray-100 px-6 py-5 space-y-3">
          {[
            { label: "SO-ID:", value: record.soId },
            { label: "State:", value: record.state.toUpperCase() },
            { label: "Country:", value: record.country.toUpperCase() },
            { label: "Agent:", value: record.agent.toUpperCase() },
            { label: "Supplier Reference", value: record.supplierReference },
            { label: "Added By:", value: record.addedBy.toUpperCase() },
            { label: "Date Sent:", value: record.date },
          ].map(({ label, value }) => (
            ...
            <div className="p-8">
              ...
              <div className="border border-gray-200 rounded-md overflow-hidden mb-6 flex flex-col">
                <div className="flex border-b border-gray-200">
                  <div className="w-1/2 p-3.5 text-sm text-gray-600 border-r border-gray-200 bg-white">SO-ID:</div>
                  <div className="w-1/2 p-3.5 text-sm text-gray-800 bg-white">{record.soId}</div>
                </div>
                <div className="flex border-b border-gray-200">
                  <div className="w-1/2 p-3.5 text-sm text-gray-600 border-r border-gray-200 bg-white">Source Warehouse:</div>
                  <div className="w-1/2 p-3.5 text-sm text-gray-800 bg-white">{record.state}, {record.country}</div>
                </div>
                <div className="flex border-b border-gray-200">
                  <div className="w-1/2 p-3.5 text-sm text-gray-600 border-r border-gray-200 bg-white">Target Agent:</div>
                  <div className="w-1/2 p-3.5 text-sm text-gray-800 bg-white">{record.agent}</div>
                </div>
                <div className="flex">
                  <div className="w-1/2 p-3.5 text-sm text-gray-600 border-r border-gray-200 bg-white">Date Sent:</div>
                  <div className="w-1/2 p-3.5 text-sm text-gray-800 bg-white">{record.date}</div>
                </div>
              </div>
