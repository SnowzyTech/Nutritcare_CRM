  ...
  return (
    <div className="flex-1 flex items-center justify-center" style={{ minHeight: "60vh" }}>
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex flex-col items-center gap-3 px-20 py-12 min-w-[360px]">
        ...
        <p className="text-xs text-gray-400">No stock transfers recorded yet.</p>
        <button
          onClick={onAdd}
...
export function TransferClient({ initialRows }: { initialRows: StockTransferRow[] }) {
            ...
            <table className="w-full min-w-[1000px] border-collapse">
              ...
              <tbody>
                {filtered.map((row) => (
                  <tr
                    key={row.id}
                    onClick={() => router.push(`/inventory/transfer/${row.id}`)}
