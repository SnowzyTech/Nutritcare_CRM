  ...
  return (
    <div className="mb-4 text-left">
      ...
      <div className="flex items-stretch gap-2">
        {showCountryCode && countryCodeValue && (
          <div className="relative flex-shrink-0 min-w-[100px] border border-gray-300 rounded-md bg-white transition-all flex items-center justify-center px-3">
...
export default function OrderFormClient({
  ...
  const [form] = useState<SavedForm | null>(initialForm);
  ...
  // The market this form was built for. `data` is only derived further down
  // (after the hooks), so read the raw blob here.
  const [country, setCountry] = useState<string>(() => {
    const saved = (initialForm?.data as Record<string, unknown> | undefined)?.country;
    return typeof saved === "string" && isSelectableCountry(saved) ? saved : DEFAULT_COUNTRY;
  });
  // Derived, never picked: the customer changes the country, not the code.
  ...
  // Initialise the funnel tab + default package/payment selections from the
  // server-provided form. The tab/index come from the URL query, which is only
  // available in the browser, so this runs once on mount.
  useEffect(() => {
    if (!form) return;
  ...
  const submitOrder = async (e: React.FormEvent) => {
    ...
    // The dial code is always stored, whatever `showCountryCode` says — that
    // setting only controls whether the customer sees the prefix. Saving a bare
    // local number produced something WhatsApp could not route.
    const withCountryCode = (raw: string) => {
      const num = (raw ?? "").replace(/\s+/g, "").replace(/^0/, "");
      if (!num) return "";
      return `${dialCode}${num}`.replace(/\s+/g, "");
    };

    const phone = withCountryCode(formValues.phone ?? "");
    const whatsapp = withCountryCode(formValues.whatsapp ?? "");
  ...
  if (funnelStep === "success") {
    ...
    const customerPhone = formValues.phone
      ? `${showCountryCodeEnabled ? dialCode + " " : ""}${formValues.phone}`
      : optinValues.whatsapp
      ? `${showCountryCodeEnabled ? dialCode + " " : ""}${optinValues.whatsapp}`
      : "";
                ...
                {formValues.whatsapp && (
                  <div className="flex justify-between py-2.5">
                    ...
                    <span className="font-semibold text-gray-800 font-mono">
                      {showCountryCodeEnabled ? dialCode + " " : ""}{formValues.whatsapp}
                    </span>
                  </div>
                )}
                {formValues.state && (
                          ...
                          <div key={key}>
                            <div className="mb-4 text-left">
                              ...
                              <select
                                value={country}
                                onChange={(e) => {
                                  // The state list and the dial code both derive
                                  // from this, so clearing the stale pick is all
                                  // that's needed.
