  ...
  return (
    <div className="space-y-6">
            ...
            <table className="w-full text-xs text-left">
              <thead className="bg-[#faf5ff] text-gray-500 uppercase font-semibold">
                <tr>
                  ...
                  <th className="px-4 py-4">Company / Agent Name</th>
              ...
              <tbody className="divide-y">
                {filteredAgents.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-4 py-10 text-center text-gray-400">No agents found</td>
                  </tr>
                ) : (
                  filteredAgents.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-gray-700 font-medium">{item.companyName}</td>
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${item.status === "ACTIVE" ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-600"}`}>
                          {item.status}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-gray-600">{item.addedBy.name}</td>
                    </tr>
                  ))
                )}
              </tbody>
            ...
            <table className="w-full text-xs text-left">
              ...
              <tbody className="divide-y">
                {filteredDrivers.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="px-4 py-10 text-center text-gray-400">No drivers found</td>
                  </tr>
                ) : (
                  filteredDrivers.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-gray-600">{item.vehicleNo ?? "—"}</td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${item.status === "ACTIVE" ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-600"}`}>
                          {item.status}
                        </span>
      ...
      <div className="flex justify-end gap-2 pb-8">
        <Button variant="outline" className="h-8 text-xs bg-gray-200 text-gray-500 border-none hover:bg-gray-300 px-4">Previous</Button>
        <Button className="h-8 w-8 text-xs bg-[#ad1df4] hover:bg-[#8e14cc] text-white rounded-md">1</Button>
        <Button variant="outline" className="h-8 text-xs bg-gray-200 text-gray-500 border-none hover:bg-gray-300 px-4">Next</Button>
      </div>
    </div>
  );
}
