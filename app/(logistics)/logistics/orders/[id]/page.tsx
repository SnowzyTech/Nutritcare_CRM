...
import { LogisticsOrderDetailClient } from "./order-detail-client";
...
export default async function LogisticsOrderDetailPage({ params }: Props) {
  ...
  const rawOrder = await getOrderWithDetails(id);
  ...
  const order = {
    ...
    date: rawOrder.date.toISOString(),
    customer: {
  ...
  return <LogisticsOrderDetailClient order={order} />;
}
