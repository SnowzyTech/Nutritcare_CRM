  ...
  return (
    <div className="max-w-6xl mx-auto flex flex-col gap-10 pb-20">
      ...
      <div>
        ...
        <div className="flex items-center gap-8 border-b border-gray-100 pb-4 overflow-x-auto no-scrollbar mb-6">
          <div className="flex items-center gap-2 text-gray-900 font-bold text-sm shrink-0">
            All
            <span className="bg-[#D6BBFB] text-white text-[10px] px-2 py-0.5 rounded-lg">{rep.orders.all}</span>
          </div>
          <div className="text-gray-600 font-bold text-sm shrink-0">
            Pending({rep.orders.pending})
          </div>
          <div className="text-gray-600 font-bold text-sm shrink-0">
            Confirmed({rep.orders.confirmed})
          </div>
          <div className="text-gray-600 font-bold text-sm shrink-0">
            Delivered({rep.orders.delivered})
          </div>
          <div className="text-gray-600 font-bold text-sm shrink-0">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-gray-400 font-medium">{rep.role}</span>
                  <span className="text-gray-600 font-bold">{rep.team}</span>
                </div>
            ...
            <div className="text-right flex items-center gap-2">
              <p className="text-xs text-gray-400 font-medium text-right mr-2">
                {rep.name.split(" ")[0]}'s KPI for this<br />month is{" "}
                <strong className="text-gray-800 text-sm">XXXXX</strong>
              </p>
              <div className="w-px h-10 bg-gray-200 mx-2"></div>
              <div className="text-right ml-2">
                <span className="text-green-500 font-black text-xl leading-none block">{rep.kpiAchieved}%</span>
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
            <div className="flex flex-wrap gap-8 lg:gap-12 flex-1">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-gray-600">{rep.team}</p>
              </div>
            </div>
      ...
      <AnalyticsSection
        repId={repId}
        repName={rep.name}
        defaultAnalytics={rep.analytics}
      />
    </div>
  );
}
