...
import type { CashFlowReport, CategoryExpenseGroup } from '@/modules/finance/services/reports-accounting.service';
...
const EMPTY: CashFlowReport = { expenseGroups: [] };
          ...
          {report.expenseGroups.length === 0 ? (
            <tr>
              <td colSpan={3} className="py-10 px-4 text-center italic text-gray-400 text-[13px]">
                No cash flow expenses recorded for the selected period. Create expense categories
                tagged as &quot;Cash Flow Statement&quot; and record expenses against them.
              </td>
            </tr>
          ) : (
            <>
              {report.expenseGroups.map((group: CategoryExpenseGroup, gi: number) => (
                <Fragment key={`cf-group-${gi}`}>
                  ...
                  {group.items.map((item, ii) => (
                    <tr key={`cf-item-${gi}-${ii}`} className="border-b border-gray-50 hover:bg-gray-50">
              ...
              <tr className="bg-[#107C41] text-white font-bold">
                <td className="py-3 px-4 uppercase">NET TOTAL CASH FLOW</td>
