...
import { OrderDetail, SALES_REPS } from '@/lib/mock-data/data-analysis';
...
const TAB_ORDER = ['All', 'Pending', 'Confirmed', 'Delivered', 'Cancelled', 'Failed'];

export function OrderDetailClient({ order }: OrderDetailClientProps) {
  const router = useRouter();
  const [isAgentModalOpen, setIsAgentModalOpen] = useState(false);
  const rep = SALES_REPS.find(r => r.name === order.repName) || SALES_REPS[0];
  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto space-y-10 pb-20">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <div className="relative w-12 h-12 rounded-full overflow-hidden border-2 border-white shadow-md">
            <Image src={rep.avatar} alt={rep.name} fill className="object-cover" sizes="48px" />
          </div>
          <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        {/* Right Col - Visual Info */}
        <div className="space-y-10">
          {/* Product Image Card */}
          <div className="relative rounded-3xl overflow-hidden shadow-2xl group border border-gray-100">
             <div 
               className="w-full aspect-[16/10] flex items-center justify-center text-white font-black text-2xl"
               style={{ backgroundColor: order.product.imageColor }}
             >
                <Image 
                  src={`https://placehold.co/400x250/${order.product.imageColor.replace('#','')}/ffffff?text=${order.product.name}`}
