...
import { X, Phone, Mail, MapPin, Star, Package, Clock } from 'lucide-react';
import { AGENT_DETAILS } from '@/lib/mock-data/data-analysis';
...
export function AgentInfoModal({ agentName, isOpen, onClose }: AgentInfoModalProps) {
  const agent = (AGENT_DETAILS as any)[agentName];
              ...
              <div className="flex items-center justify-center gap-1 mt-1">
                {[...Array(5)].map((_, i) => (
                  <Star 
                    key={i} 
                    size={14} 
                    className={i < Math.floor(agent.rating) ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200'} 
                  />
                ))}
                <span className="text-xs font-bold text-gray-500 ml-1">{agent.rating} Rating</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl border border-gray-100">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-sm font-bold text-gray-800">{agent.location}</span>
              </div>
            </div>
          </div>
