...
export function OrderDashboardClient({ initialOrders = [], repProfile }: OrderDashboardClientProps) {
  ...
  return (
    <div className="p-8 max-w-[1400px] mx-auto">
      ...
      {/* Tabs */}
      <div className="flex items-center justify-between mb-8 bg-white rounded-xl shadow-sm p-1.5 overflow-x-auto no-scrollbar gap-2 w-full">
        {TABS.map((tab) => {
          ...
          return (
            <button
              ...
              className={`relative px-6 py-2.5 rounded-lg transition-all duration-200 flex items-center justify-center gap-1.5 whitespace-nowrap flex-1 hover:cursor-pointer ${isActive ? 'bg-[#F9F5FF] text-[#6941C6]' : 'text-gray-500 hover:bg-gray-50'}`}
            >
              <span className={`text-sm font-medium ${isActive ? 'text-[#6941C6] font-bold' : 'text-gray-500'}`}>
                {tab}{!isActive && tab !== 'All' && count > 0 ? `(${count})` : ''}
              </span>
              {tab === 'All' && (
      ...
      <div className="flex flex-col items-start gap-6 justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <button className="p-2 text-gray-400 hover:cursor-pointer hover:text-gray-600">
            <ArrowUpDown size={18} />
          </button>
        </div>
      ...
      <div className="bg-[#F8F9FA] rounded-2xl overflow-x-auto">
        <table className="w-full min-w-[1100px] text-left border-separate border-spacing-y-0">
          <thead>
            <tr>
              <th className="px-8 py-5 text-[11px] font-bold text-gray-400 uppercase tracking-wider whitespace-nowrap">G-Mail</th>
              <th className="px-8 py-5 text-[11px] font-bold text-gray-400 uppercase tracking-wider whitespace-nowrap">Name</th>
              <th className="px-8 py-5 text-[11px] font-bold text-gray-400 uppercase tracking-wider whitespace-nowrap">Agent</th>
              <th className="px-8 py-5 text-[11px] font-bold text-gray-400 uppercase tracking-wider whitespace-nowrap">Product</th>
              <th className="px-8 py-5 text-[11px] font-bold text-gray-400 uppercase tracking-wider text-center whitespace-nowrap">Quantity</th>
              <th className="px-8 py-5 text-[11px] font-bold text-gray-400 uppercase tracking-wider text-right whitespace-nowrap">Date</th>
              <th className="px-8 py-5 text-[11px] font-bold text-gray-400 uppercase tracking-wider whitespace-nowrap">Status Date</th>
            </tr>
          </thead>
          <tbody className="bg-white">
            {filteredOrders.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-8 py-12 text-center text-sm text-gray-400">No orders found</td>
              </tr>
